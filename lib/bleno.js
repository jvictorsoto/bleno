...
Bleno.prototype.startAdvertisingDirect = function(directAddrType, directAddr, callback) {
  if (this.state !== 'poweredOn') {
    var error = new Error('Could not start advertising, state is ' + this.state + ' (not poweredOn)');
    ...
    this._bindings.startAdvertisingDirect(directAddrType, directAddr);
  }
};
...
Bleno.prototype.setServices = function(services, genBaseServices, callback) {
  if (callback) {
    this.once('servicesSet', callback);
  }
  this._bindings.setServices(services, genBaseServices);
};
