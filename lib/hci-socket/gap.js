...
var HCI_STATUS_MAPPER = require('./hci-status');
...
Gap.prototype.startAdvertisingDirect = function(directAddrType, directAddr) {
  this._advertiseState = 'starting';
  this._hci.setScanResponseData(Buffer.from([]));
  this._hci.setAdvertisingData(Buffer.from([]));
  this._hci.setAdvertisingParameters(0x01, 0x00, directAddrType, directAddr); // Connectable Directed Advertising
...
Gap.prototype.startAdvertising = function(name, serviceUuids) {
  ...
  advertisementData.writeUInt8(0x05, 2);
...
Gap.prototype.onHciLeAdvertiseEnableSet = function(status) {
  if (this._advertiseState === 'starting') {
    ...
    if (status) {
      error = new Error(HCI_STATUS_MAPPER[status] || ('Unknown (' + status + ')'));
    }
