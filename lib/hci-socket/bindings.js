...
var BlenoBindings = function(config) {
  this._config = config;
  this._state = null;

  this._advertising = false;

  this._hci = new Hci(this._config);
  this._gap = new Gap(this._hci, this._config);
  this._gatt = new Gatt(this._hci, this._config);
...
BlenoBindings.prototype.startAdvertisingDirect = function(directAddrType, directAddr) {
  this._advertising = true;

  this._gap.startAdvertisingDirect(directAddrType, directAddr);
};
...
BlenoBindings.prototype.setServices = function(services, genBaseServices) {
  this._gatt.setServices(services, genBaseServices);
...
BlenoBindings.prototype.onLeConnComplete = function(status, handle, role, addressType, address, interval, latency, supervisionTimeout, masterClockAccuracy) {
  ...
  this._handle = handle;
  this._aclStream = new AclStream(this._hci, handle, this._hci.addressType, this._hci.address, addressType, address, this._config.smpPassKey);
