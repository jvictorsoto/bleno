...
Hci.prototype.init = function() {
  ...
  if (this._config.hciChannelUser) {
    this._deviceId = this._socket.bindUser(this._config.deviceId);

    this._socket.start();

    this.reset();
  } else {
    this._deviceId = this._socket.bindRaw(this._config.deviceId);
    this._socket.start();
...
Hci.prototype.setAdvertisingParameters = function(type = 0x00, ownAddrType = 0x00, directAddrType = 0x00, directAddr = '00:00:00:00:00:00') {
  ...
  cmd.writeUInt8(type, 8); // adv type
  cmd.writeUInt8(ownAddrType, 9); // own addr typ
  cmd.writeUInt8(directAddrType, 10); // direct addr type
  (new Buffer(directAddr.split(':').reverse().join(''), 'hex')).copy(cmd, 11); // direct addr
